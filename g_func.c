...
void plat_blocked(edict_t* self, edict_t* other)
{
	if (!(other->svflags & SVF_MONSTER) && (!other->client))
	{
		...
		T_Damage(other, self, self->activator, vec3_origin, other->s.origin, vec3_origin, 100000, 100000, 1, DAMAGE_NO_BLOOD, MOD_CRUSH);
	...
	T_Damage(other, self, self->activator, vec3_origin, other->s.origin, vec3_origin, self->dmg, self->dmg, 1, DAMAGE_NO_BLOOD, MOD_CRUSH);
...
void Touch_Plat_Center(edict_t* ent, edict_t* other, cplane_t* plane, csurface_t* surf)
{
	...
	if (((int)gibflags->value & GF_MUST_USE) && !(other->client->buttons & BUTTON_USE))
		return;
...
void button_touch(edict_t* self, edict_t* other, cplane_t* plane, csurface_t* surf)
{
	...
	if ((int)gibflags->value & GF_MUST_LOOK)
	{
		vec3_t	vec;
		float	dot;
		vec3_t	forward;
		...
		VectorMA(self->absmin, 0.5, self->size, vec);
		VectorSubtract(vec, other->s.origin, vec);
		VectorNormalize(vec);
...
void button_trigger_touch(edict_t* self, edict_t* other, cplane_t* plane, csurface_t* surf)
{
	// SLUGFILLER--convey the message
	if (self->owner->touch)
		self->owner->touch(self->owner, other, plane, surf);
}
...
void SP_func_button(edict_t* ent)
{
	...
	// SLUGFILLER--the button needs a trigger for the use operation
	if ((int)gibflags->value & GF_MUST_USE)
	{
		edict_t* trigger;
		...
		trigger->solid = SOLID_TRIGGER;
		trigger->movetype = MOVETYPE_NONE;
		trigger->touch = button_trigger_touch;

		VectorCopy(ent->absmin, trigger->mins);
		VectorCopy(ent->absmax, trigger->maxs);
		trigger->mins[0]--;
		trigger->mins[1]--;
		trigger->maxs[0]++;
		trigger->maxs[1]++;
...
void use_killbox(edict_t* self, edict_t* other, edict_t* activator)
{
	...
	while (1)
	{
		tr = gi.trace(self->s.origin, self->mins, self->maxs, self->s.origin, NULL, MASK_PLAYERSOLID);
		if (!tr.ent)
			break;

		// nail it
		T_Damage(tr.ent, self, activator, vec3_origin, self->s.origin, vec3_origin, 100000, 100000, 0, DAMAGE_NO_PROTECTION | DAMAGE_NO_BLOOD, MOD_TELEFRAG);
