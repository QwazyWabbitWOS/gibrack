...
=================
*/
// SLUGFILLER--gib touch wasn't working anyway so it was removed
...
void ThrowGib(edict_t* self, char* gibname, int damage, int type)
{
	...
	// SLUGFILLER--no gib velocity clipping
...
void blood_touch(edict_t* self, edict_t* other, cplane_t* plane, csurface_t* surf)
{
	...
	self->s.frame++;
	self->think = blood_think;
	self->nextthink = level.time + FRAMETIME;
}

// SLUGFILLER--buffer for overflaw control
void InitBloodyQue(void)
{
	int		i;
	edict_t* ent;

	globals.num_edicts += 30;
	level.bloody_que = 0;
	ent = &g_edicts[(int)maxclients->value * 2 + 1];
	for (i = 0; i < 30; i++, ent++)
	{
		G_InitEdict(ent);
		ent->classname = "bloodyque";
		ent->solid = SOLID_TRIGGER;
		ent->clipmask = MASK_SOLID | MASK_WATER;
		ent->s.modelindex = gi.modelindex("models/objects/gibs/blood/tris.md2");
	}
}

void ThrowBlood(vec3_t origin, vec3_t dir)
{
	edict_t* blood;

	// SLUGFILLER--add blood flying out of the victim
	blood = &g_edicts[(int)maxclients->value * 2 + level.bloody_que + 1];
	level.bloody_que = (level.bloody_que + 1) % 30;
	blood->nextthink = 0;
	blood->groundentity = NULL;
	blood->owner = NULL;
	blood->s.frame = 0;
	blood->num_clusters = 0;

	VectorCopy(origin, blood->s.old_origin);
	VectorCopy(origin, blood->s.origin);
	VectorCopy(dir, blood->velocity);
	blood->movetype = MOVETYPE_TOSS;
	blood->touch = blood_touch;
	blood->avelocity[0] = random() * 600;
	blood->avelocity[1] = random() * 600;
	blood->avelocity[2] = random() * 600;
}
...
void func_object_touch(edict_t* self, edict_t* other, cplane_t* plane, csurface_t* surf)
{
	...
	if (other->takedamage == DAMAGE_NO)
		return;
	T_Damage(other, self, self, vec3_origin, self->s.origin, vec3_origin, self->dmg, self->dmg, 1, DAMAGE_NO_BLOOD, MOD_CRUSH);
}
...
// SLUGFILLER--delay damage instead of explosion
void barrel_damage(edict_t* self)
{
	T_RadiusDamage(self->owner, self->owner->activator, self->owner->dmg, NULL, self->owner->dmg + 40, MOD_BARREL);
}
...
void barrel_explode(edict_t* self)
{
	...
	self->nextthink = level.time + 3 * FRAMETIME;
