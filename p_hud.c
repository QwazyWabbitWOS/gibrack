...
void G_SetStats(edict_t* ent)
{
	...
	//
	// SLUGFILLER--powerups and timers
	//
	ent->client->ps.stats[STAT_TIMER_ICON] = 0;
	ent->client->ps.stats[STAT_TIMER] = 0;

	// SLUGFILLER--phase one - count the powerups
	powerups = 0;
	for (index = 1; index < game.num_items; index++)
	{
	...
	if (powerups)
	{
		// SLUGFILLER--phase two - pick one
		powerups = ((level.framenum >> 3) % powerups) + 1;

		// SLUGFILLER--phase three - show it
		for (index = 1; index < game.num_items; index++)
		{
			if (!(itemlist[index].flags & IT_POWERUP))
				continue;
	...
	if (lives->value > 0)
	{
		ent->client->ps.stats[STAT_LIVES_ICON] = level.pic_health;
		if (ent->client->pers.lives <= 9)
