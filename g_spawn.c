...
void SpawnEntities (char *mapname, char *entities, char *spawnpoint)
{
	...
	if (lives_value > 9)
		lives_value = 9;
	if (lives->value != lives_value)
		gi.cvar_forceset("lives", va("%f", lives_value));
...
// parse ents
	while (1)
	{
		...
		if (ent != g_edicts)
		{
			if (deathmatch->value && strncmp(ent->classname, "monster_", 8) != 0)
			{
	...
	// SLUGFILLER--this is a little bit premature but it is done so all coop
	// players use the same intermission
	...
	// SLUGFILLER--in cinematics and demos there aren't any entities.
	// in that case ent is NULL and setting values in it would cause
	// a general protection fault. we don't want that.
	if (ent)
	{
...
void SP_worldspawn (edict_t *ent)
{
	...
	// SLUGFILLER--reserve spots for blood overflaw control
	InitBloodyQue ();
	...
	gi.soundindex ("misc/hshot.wav");		// head shot

	sm_meat_index = gi.modelindex ("models/objects/gibs/sm_meat/tris.md2");
	gi.modelindex ("models/objects/gibs/blood/tris.md2");
