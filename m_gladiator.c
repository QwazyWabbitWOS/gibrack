...
void GaldiatorMelee (edict_t *self)
{
	...
	if (fire_hit (self, aim, (20 + (rand() %5)), 60))
...
void gladiator_attack(edict_t *self)
{
	...
	// SLUGFILLER--saving will not be neccecery thanks to the aimangles method
...
void SP_monster_gladiator (edict_t *self)
{
	...
	VectorSet (self->headmins, -32, -32, 30);
	VectorSet (self->headmaxs, 32, 32, 64);
	...
	self->monsterinfo.speed = 18;
	self->monsterinfo.walkspeed = 7;
